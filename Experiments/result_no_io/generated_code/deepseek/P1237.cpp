...
bool is_redundant(int index, bool used[MAX_DEPENDENCIES], int path[MAX_DEPENDENCIES], int path_len) {
    char target[MAX_DOMAIN_LENGTH];
    strcpy(target, dependencies[index].rhs);
    char current[MAX_DOMAIN_LENGTH];
    strcpy(current, dependencies[index].lhs);
    ...
    for (int i = 0; i < n; i++) {
        ...
        if (strcmp(dependencies[i].lhs, current) == 0) {
            ...
            if (is_redundant(index, used, path, path_len)) {
                return true;
            }
            used[i] = false;
            path_len--;
