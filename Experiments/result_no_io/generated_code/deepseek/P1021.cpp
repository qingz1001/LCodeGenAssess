...
void dfs(int depth, int last_value) {
    if (depth == K) {
        for (int i = 0; i <= max_value; i++) dp[i] = MAX_N + 1;
        dp[0] = 0;

        for (int i = 0; i < K; i++) {
            for (int j = current_values[i]; j <= max_value; j++) {
                dp[j] = (dp[j] < dp[j - current_values[i]] + 1) ? dp[j] : (dp[j - current_values[i]] + 1);
            }
        }

        int max_possible = 0;
        while (dp[max_possible + 1] <= N) max_possible++;

        if (max_possible > max_value) {
            max_value = max_possible;
            for (int i = 0; i < K; i++) best_values[i] = current_values[i];
        }
        return;
    }

    for (int i = last_value + 1; i <= max_value + 1; i++) {
