...
int main() {
    ...
    while (m--) {
        ...
        for (int i = a; i <= b; i++) {
            int pos = rk[i];
            if (pos >= l && pos <= r) {
                ans = max(ans, min(b - i + 1, query(l + 1, pos)));
            } else if (pos < l) {
                ans = max(ans, min(b - i + 1, lcp[l]));
            } else {
                ans = max(ans, min(b - i + 1, lcp[pos]));
            }
        }
        printf("%d\n", ans);
    }
